defmodule AuroraMeter.Storage.Ecto do
  ...
  **Internal.** Not part of the supported API (see [API inventory](api.md)).
  ...
  # -- the bind-parameter ceiling (X338) --------------------------------------

  # Postgres carries a statement's bind-parameter count in a **16-bit** field of
  # the wire protocol's Bind message, so one statement takes at most 65,535 of
  # them. `insert_all/3` sends one parameter per column per row, so every
  # multi-row insert in this adapter has a row ceiling, and it is not a soft
  # one: Postgrex refuses to encode the statement and the query never reaches
  # the database at all.
  #
  # **X338** is what that cost. `add_counters/1` built one `insert_all` over a
  # whole flush batch at seven parameters per counter row, so a flush of more
  # than `floor(65535 / 7) = 9,362` distinct counter keys could not be sent;
  # `AuroraMeter.Flusher.failed/2` retained the batch for an idempotent retry,
  # which is right for a transient failure and exactly wrong for this one; and
  # the node buffered for ever with no way to reach the database. At the shipped
  # five second `flush_interval` the threshold was 1,873 distinct keys a second.
  #
  # The fix is to chunk, and the chunk size is **derived from the schema and
  # never written down**. A column added to `AuroraMeter.Schema.Counter` in a
  # later version lowers the real limit, and a literal here would keep the old
  # one and fail again at a different row count, which is the same defect with a
  # new number. `__schema__(:fields)` counts every field the schema declares,
  # including the autogenerated primary key that `insert_all/3` does send, so it
  # is an **upper bound** on what a row can cost: an entry map that omits a
  # field only makes the chunk smaller than it had to be, never larger.
  @max_bind_params 65_535

  @doc """
  The most rows of `schema` one `insert_all/3` statement can carry.

  Derived from the schema, because Postgres's 65,535 bind-parameter ceiling is
  per statement and `insert_all/3` spends one parameter per column per row
  (`open-findings.md` X338). Public so that a test can flush one row more than
  this against a real database rather than trusting the arithmetic.

  ## Examples

      iex> AuroraMeter.Storage.Ecto.rows_per_statement(AuroraMeter.Schema.Counter)
      9362

  """
  @spec rows_per_statement(module()) :: pos_integer()
  def rows_per_statement(schema) do
    div(@max_bind_params, length(schema.__schema__(:fields)))
  end

  # Chunking turns one atomic statement into several, and the flusher's retry
  # contract rests on a batch being all-or-nothing: a half-applied flush that is
  # then retried double counts the half that landed.
  #
  # Inside `flush_batch/3` nothing changes, because the receipt and both delta
  # sets are already inside one `repo().transaction/1` and Ecto joins a nested
  # `transaction/1` to the open one rather than opening a second. The receipt
  # still decides idempotency and still commits or rolls back with every chunk.
  #
  # Outside it, `add_counters/1` and `add_history/1` are public storage
  # callbacks whose callers have always been able to assume "all of these deltas
  # or none", because one statement gave them that for free. So a call that
  # needs more than one chunk opens the transaction the single statement used to
  # be, and a call that fits in one takes exactly the path it always took: the
  # flush of a thousand keys is the same round trips it was.
  defp atomically([], fun), do: fun.([])
  defp atomically([_only_one] = chunks, fun), do: fun.(chunks)

  defp atomically(chunks, fun) do
    {:ok, result} = repo().transaction(fn -> fun.(chunks) end)
    result
  end

  # `Enum.chunk_every/2` on an empty list is `[]`, and `Enum.flat_map/2` over
  # `[]` never runs the insert, which is what the empty case needs anyway
  # (`AuroraMeter.Storage.add_counters/1` short-circuits `[]` before it gets
  # here, but this adapter is also called directly).
  defp in_chunks(entries, schema, fun) do
    entries
    |> chunks_of(schema)
    |> atomically(fn chunks -> Enum.flat_map(chunks, fun) end)
  end

  # The split on its own, for the call sites that are already inside a
  # transaction their caller opened and must not open a second.
  defp chunks_of(entries, schema), do: Enum.chunk_every(entries, rows_per_statement(schema))
  ...
  def upsert_counters(rows) do
    ...
    in_chunks(entries, Counter, fn chunk ->
      repo().insert_all(Counter, chunk,
  ...
  def add_history(rows) do
    ...
    returned =
      in_chunks(entries, History, fn chunk ->
        {_count, returned} =
          repo().insert_all(History, chunk,
            on_conflict:
              from(h in History,
                update: [
                  set: [
  ...
  def insert_events(rows) do
    ...
    in_chunks(entries, Event, fn chunk ->
  ...
  defp ensure_total_rows(generation, entries, now) do
    ...
    in_chunks(zeroes, EventTotal, fn chunk ->
      repo().insert_all(EventTotal, chunk,
        on_conflict: :nothing,
  ...
  defp insert_events_returning(repo, entries, timeout) do
    ...
    # Chunked for the same reason the flush path is (X338), and without a
    # transaction of its own because every caller is already inside one. An
    # event row costs one bind parameter per column of a wide schema, so one
    # statement stops at a couple of thousand events while `record_batch/2`
    # accepts a list of any length. The sort above is over the WHOLE batch and
    # the split comes after it, so the chunks still take rows in one total order
    # and L-03b-3's deadlock argument is unchanged.
    returned =
      rows
      |> chunks_of(Event)
  ...
  defp apply_totals(repo, events, generations, timeout) do
    ...
    # Chunked (X338), inside the caller's transaction, and after the sort for
    # the same total-order reason as the event insert above.
    Enum.each(chunks_of(entries, EventTotal), fn chunk ->
      repo.insert_all(EventTotal, chunk,
        on_conflict:
          from(t in EventTotal,
            update: [
