defmodule AuroraMeter.Migration do
  ...
  ## Versions
    ...
    * **10**: plan version identity. `aurora_meter_plan_versions` (the snapshot
      of each plan version's commercial content, written by
      `AuroraMeter.Plans.register!/0` and read only to interpret a version whose
      block has been deleted from the host's plans module),
      `aurora_meter_plan_transitions` (created here, written by a later release),
      and ten columns on `aurora_meter_subscriptions`, of which `plan_version`
      is the one the version exists for: it pins a tenant to the contract they
      were sold, so redeploying an edited plan block is a refused boot rather
      than a silent repricing. Additive and writes no data; the legacy
      assignment of `plan_version = '1'` is done by
      `AuroraMeter.Plans.register!/0` at the first boot after the upgrade,
      because the fingerprint it writes beside it is a digest of the compiled
      definition and cannot be computed in SQL. It also sets a
      `clock_timestamp()` default on `aurora_meter_flush_receipts.inserted_at`,
      so a receipt is stamped by the database that later compares it rather
      than by the node that wrote it
