...
| `expire_reserved_grant` | a promotional grant a hold had reserved, which the expiry sweep then destroyed in full. The sweep's guard is the whole wallet's `balance - held`, so another grant covering the held amount lets it reach this one | nothing to repair: the wallet's own history is inconsistent about which grant the hold was holding. It keeps working on the legacy writer |
