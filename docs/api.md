...
| Entry | Signature and return | Class | Since | Notes |
|---|---|---|---|---|
| `AuroraMeter.Events.get/2` | `(tenant, String.t()) :: {:ok, AuroraMeter.Event.t()} \| {:error, :not_found \| {:invalid, [{atom(), atom()}]} \| {:conflict, AuroraMeter.Event.t()} \| {:unavailable, term()} \| {:unsupported, :durable_events}}` | stable | 1.0.0 | By the caller's own `event_id`. |
...
| `AuroraMeter.Events.Replay.run/1` | `(keyword()) :: {:ok, map()} \| {:ok, :paused, map()} \| {:error, {:projection_mismatch, map()}} \| {:error, {:already_running, map()}} \| {:error, term()}` | stable | 1.0.0 | Rebuilds the projection into a new generation and activates it. Options `:batch_size`, `:compare`, `:activate`, `:resume`, `:generation`, `:compare_limit`, `:max_batches`, `:rehydrate`, `:timeout`. An operator action; nothing schedules it. |
...
| `AuroraMeter.Events.Replay.prune/1` | `(integer()) :: {:ok, non_neg_integer()} \| {:error, :active \| :not_found \| {:already_running, map()}} \| {:error, term()}` | stable | 1.0.0 | Deletes a retired generation's rows in bounded slices. Refuses the active one and refuses while a replay holds its claim. |
...
| `AuroraMeter.Storage.record_events/2` | `([event_entry()], keyword()) :: {:ok, [{AuroraMeter.Event.t(), :inserted \| :duplicate}]} \| {:error, {:conflict, non_neg_integer(), AuroraMeter.Event.t()}} \| {:error, term()}` | stable | 1.0.0 | One transaction for the events, their totals and the outbox intent. |
| `AuroraMeter.Storage.record_correction/2` | `(correction_entry(), keyword()) :: {:ok, [{AuroraMeter.Event.t(), :inserted \| :duplicate}]} \| {:error, {:conflict, non_neg_integer(), AuroraMeter.Event.t()}} \| {:error, {:invalid, [{atom(), atom()}]}} \| {:error, {:not_found, :original}} \| {:error, term()}` | stable | 1.0.0 | One correction, and optionally its replacement, in one transaction. The duplicate check precedes the cumulative bound, which is taken under a lock on the original. Options `:timeout`, `:outbox`, `:replacement`. |
...
| `AuroraMeter.Storage.put_plan_version/1` | `(map()) :: {:ok, PlanVersion.t()} \| {:error, term()} \| {:error, {:unsupported, capability()}}` | additive | 1.0.0 | Insert or nothing. Never an update: a snapshot is immutable. Needs the `:plan_versions` capability. |
| `AuroraMeter.Storage.list_plan_versions/1` | `(String.t() \| :all) :: [PlanVersion.t()] \| {:error, {:unsupported, capability()}}` | additive | 1.0.0 | Needs the `:plan_versions` capability. |
| `AuroraMeter.Storage.assign_legacy_plan_versions/1` | `(pos_integer()) :: {:ok, %{assigned: non_neg_integer(), orphans: non_neg_integer()}} \| {:error, term()} \| {:error, {:unsupported, capability()}}` | additive | 1.0.0 | One bounded transaction of the legacy assignment, taken with `FOR UPDATE SKIP LOCKED`. `assigned: 0` means there is nothing left to do. Needs the `:plan_versions` capability. |
...
| `AuroraMeter.Subscriptions.schedule_transition/3` | `(tenant, atom(), keyword()) :: {:ok, PlanTransition.t()} \| {:error, {:invalid \| :conflict \| :not_found \| :unavailable, term()}}` | additive | 1.0.0 | One scheduled plan change. `:ref` required and idempotent per tenant; `:version`, `:effective_at` (default: the end of the tenant's period), `:replace` (default `true`), `:confirm` (`:local` or `:provider`), `:detail`. |
| `AuroraMeter.Subscriptions.cancel_transition/3` | `(tenant, String.t(), keyword()) :: {:ok, PlanTransition.t()} \| {:error, {:invalid \| :conflict \| :not_found \| :unavailable, term()}}` | additive | 1.0.0 | Idempotent. An applied transition is a conflict. Arity 2 exists through a default `[]`. |
| `AuroraMeter.Subscriptions.preview_transition/3` | `(tenant, atom(), keyword()) :: {:ok, map()} \| {:error, {:invalid \| :conflict \| :not_found \| :unavailable, term()}}` | additive | 1.0.0 | A pure read: no lock, no write. Returns `from`, `to`, `changes`, `effective_at`, `period` and `provider`. `price` is the plan's list price, never an invoice amount. |
| `AuroraMeter.Subscriptions.confirm_transition/3` | `(tenant, String.t(), keyword()) :: {:ok, PlanTransition.t()} \| {:error, {:invalid \| :conflict \| :not_found \| :unavailable, term()}}` | additive | 1.0.0 | For a billing provider integration. `:provider_ref` required; `:effective_at` wins over the scheduled boundary. Idempotent under redelivery. |
