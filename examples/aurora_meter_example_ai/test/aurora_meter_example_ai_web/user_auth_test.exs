defmodule AuroraMeterExampleAiWeb.UserAuthTest do
  ...
  alias AuroraMeterExampleAiWeb.UserAuth

  import AuroraMeterExampleAi.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AuroraMeterExampleAiWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "writes a cookie if remember_me was set in previous session", %{conn: conn, user: user} do
      ...
      conn =
        ...
        |> Map.replace!(
          :secret_key_base,
          AuroraMeterExampleAiWeb.Endpoint.config(:secret_key_base)
        )
        |> fetch_cookies()
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AuroraMeterExampleAiWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AuroraMeterExampleAiWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
